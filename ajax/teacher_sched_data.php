    ...
    foreach ($pt_availability as $pt_key => $pt_value) {
        ...
        //    var_dump($array_teaching[$faculty_id]['teaching_load']) ;
            ...
            foreach ($array_teaching[$faculty_id]['teaching_load'] as $key => &$value) {
                ...
                $schedule_to = Schedule($schedule, $value['total_hours']);
// echo json_encode($schedule_to.''.$schedule.$section);
// exit;
                   
                //checking for overlapping in section
                $sec_lap = "SELECT user_id FROM temporary_sched WHERE user_id = '$user_id' AND day = '$day' AND section = '$section' AND (( '$schedule' > schedule && '$schedule_to' <  schedule_to ) OR (('$schedule' > schedule && '$schedule' < schedule_to) || ('$schedule_to' > schedule && '$schedule_to' < schedule_to)) OR ( '$schedule' = schedule || '$schedule_to' = schedule_to) OR (schedule > '$schedule' && schedule_to <'$schedule_to')) ";
// echo json_encode( $sec_lap);
// exit;
                $section_overlap = mysqli_query($db_connect, $sec_lap);
                if (mysqli_num_rows($section_overlap) > 0) { //if section have overlap
                //  echo json_encode(mysqli_num_rows($section_overlap));
                //  exit;
                ...
                //     break;
                    ...
                    $query_insert = "INSERT INTO `temporary_sched`(`room_type`,`room`,`day`, `from_time`, `to_time`, `faculty_id`, `section`, `subject`,`course_title`,`unit`,`name`, `schedule`,`schedule_to`,`user_id`,`total_hrs`) VALUES ('$room_type','5','$day','" . $fromtime . "','" . $totime . "','$faculty_id','$section','$subject','$course_title','$unit','$name','$schedule','$schedule_to','$user_id','" . $total_hrs . "')";
